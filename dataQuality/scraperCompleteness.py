...
def field_completeness(doc_list, doc_list_length, field_names):
    ...
    for doc in doc_list:
        for key in mean_std_dict.keys():
            if doc[key] == None or doc[key] == "Unknown":
                mean_std_dict[key] += 1
...
def main():
    ...
    with open("configMongoQuality.yml", "r") as file:
        ...
        with open("scraperCompletenessResults.txt", "w+") as outfile:
            json.dump(result, outfile, indent = 4, sort_keys = True)
        print("Results saved in ./scraperCompletenessResults.txt")
        logging.info("Results saved in ./scraperCompletenessResults.txt")
